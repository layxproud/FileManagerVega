...
//        QAxObject *RusGramTab;
    ...
    public:
        ...
        QString AddNorms(QStringList *list, QString term, QString context, int &Interrupt, float fProgress, bool bFindNorms = true);
        QString GetNorm(QString term, QString context, int &Interrupt, float fProgress, bool bAskUser = false);
...
public:
    ...
    EParamStructure	eParamStruct;  	// структура параметра: точный, морфология, иерархия // NormalizationID
...
struct IPExtendedCompareResults : IPCompareResults
{
    ...
    bool operator < (const IPExtendedCompareResults& str) const
    {
        return !(sim2 < str.sim2);
    }
};
...
*/									// Функция создания ИП по данным внешнего ресурса
    virtual int GenerateByText(QString aText, bool bFindNorms = true, ulong aOccurThreshold = 0,
                       double aPercentThreshold = 0., double aWeightThreshold = 0.,
