...
private:
    ...
    std::string Read(
        const std::string &section, const std::string &key, const TypeName &DefaultValue);
    template<class TypeName>
    bool Write(const std::string &section, const std::string &key, const TypeName &Value);
    ...
    std::string GetSectionName(
        const std::string &str) const; // возвращает имя секции(значение, ограниченное []) из
    // строки str
    ...
    size_t ProceedToSection(
        const std::string &section,
        std::vector<std::string> &lines); // возвращает номер строчки, следующей за заголовком
    // секции, если не найдена, то lines.size()
    bool RewriteFile(std::vector<std::string> &lines); // Перезапиь файла строчками из вектора

    bool Equal(
        std::string s1,
        std::string s2); // проверка равенства двух строк без учета регистра - ВСЕ СРАВНЕНИЯ
                         // СЕКЦИЙ/КЛЮЧЕЙ ТОЛЬКО ЧЕРЕЗ ЭТОТ МЕТОД
...
public:
  ...
  */
    int ReadInteger(
        std::string section, std::string key, int defval = 1); // чтение целочисленного значения
    bool ReadBool(
        std::string section, std::string key, bool defval = false); // чтение логического значения
    std::string ReadString(
        std::string section,
        std::string key,
        std::string defval = "error"); // чтение строкового значения
    std::string ReadStringS(
        std::string section,
        std::string key,
        std::string defval = "error",
        unsigned int strSize = sizeof("error"));
    double ReadFloat(
        std::string section, std::string key, double defval = 0.); // чтение вещественного значения
    int ReadColor(
        std::string section,
        std::string key,
        int defval = 0); // чтение значения цвета(в файле оно записано в 16-тиричной форме)
    void *ReadStruct(
        std::string section,
        std::string key,
        void *ptr,
        unsigned int
            size); // чтение структуры, принимает адрес куда писать и размер в байтах, при ошибке
                   // позвращает NULL и ничего туда не пишет
    uint64_t ReadBitSet(
        std::string section, std::string key, uint64_t defval = 0); // чтение из файла 64 бит
    ...
    std::string KeyName(
        std::string section, unsigned int KeyIndex); // имя ключа по имени секции и индексу
